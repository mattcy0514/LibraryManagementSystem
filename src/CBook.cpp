#include "CBook.h"
CBook::CBook(const std::string title, const std::string author, const std::string ISBN):
...
CBook::CBook(CBook&& other) noexcept:
    title(std::move(other.title)), author(std::move(other.author)), ISBN(std::move(other.ISBN)), status(other.status) {
    other.title = "";
    other.author = "";
    other.ISBN = "";
    other.status = false;
