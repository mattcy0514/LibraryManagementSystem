...
CUser::CUser(const CUser& other) : userID(other.userID), name(other.name)
{
}
CUser::CUser(CUser&& other) noexcept
    : userID(other.userID),
      name(std::move(other.name)),
      borrowedBooks(std::move(other.borrowedBooks)),
...
{
  book.returnBook();
  auto it = std::find_if(this->borrowedBooks.begin(), this->borrowedBooks.end(),
                         [book](CBook& found_book) {
